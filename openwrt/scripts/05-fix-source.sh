...
[ "$version" = "snapshots-24.10" ] && {
    ...
    git clone https://$github/sbwml/feeds_packages_net_haproxy feeds/packages/net/haproxy
}
...
if [ "$USE_GCC14" = y ] || [ "$USE_GCC15" = y ]; then
    ...
    # xdp-tools
    [ "$version" = "rc2" ] && curl -s https://$mirror/openwrt/patch/openwrt-6.x/gcc-14/xdp-tools/900-Fix-transposed-calloc-arguments.patch > package/network/utils/xdp-tools/patches/900-Fix-transposed-calloc-arguments.patch
    # perl
...
# openwrt 24.10
[ "$version" = "snapshots-24.10" ] && rm -rf package/network/utils/xdp-tools/patches/021-headers-xdp-drop-vlan_hdr-as-already-defined.patch
